...
describe('EmailFinder', () => {
  ...
  describe('findEmailWithApollo', () => {
    it('should successfully find email with Apollo API', async () => {
      ...
      const result = await EmailFinder.findEmailWithApollo(mockProfileData);

      expect(result.success).toBe(true);
      expect(result.email).toBe('jane.smith@google.com');
      expect(result.source).toBe('apollo');
    });

    it('should handle Apollo API failure', async () => {
      const mockProfileData = {
        name: 'John Doe',
    ...
    it('should handle Chrome runtime errors', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Extension error occurred');
    ...
    it('should handle unexpected errors gracefully', async () => {
      ...
      chrome.runtime.sendMessage.mockImplementation((message, callback) => {
        ...
        setTimeout(() => {
          if (callback) {
            callback(null); // Null response simulates connection failure
          }
        }, 100);
      });
      ...
      // The method should handle the error gracefully and return a failure result
      expect(result).toBeDefined();
      // Since callback returns null, this should be handled as a failure
    });
  });
